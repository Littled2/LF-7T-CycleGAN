...
class MRIGenerator(nn.Module):
    """
    Generator with Attention and Edge blocks, and perceptual and content adversarial loss
    ...
    def __init__(self, input_nc=1, ngf=64):
        ...
        self.attention_block = AttentionBlock(in_channels=input_nc, features=ngf)
        self.edge_block = EdgeBlock(in_channels=input_nc, features=ngf)

        # Define the laplacian kernel for edge extraction
        laplacian_kernel = [
            [ 0, 1, 0 ],
            [ 1, -4, 1 ],
            [ 0, 1, 0 ]
        ...
        laplacian_tensor = torch.tensor(laplacian_kernel, dtype=torch.float32).view(1, 1, 3, 3)
        self.register_buffer('laplacian_kernel', laplacian_tensor)


    def forward(self, x):
        """
        Forward process of the MRIGenerator

        """

        # Compute enhanced images a per the reference paper

        I_att, I_content = self.attention_block(x)

        I_edge = F.conv2d(I_att, self.laplacian_kernel, padding=1) # Extract edges using the Laplacian kernel

        I_edge_refined = self.edge_block(I_edge)

        HF_images = I_att + (I_edge_refined - I_edge)

        return HF_images, I_content # Return enhanced image and I_content for loss calculation
