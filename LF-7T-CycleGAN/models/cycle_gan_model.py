...
class CycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        # Define command line options. The loss function lambdas here were adapted for LF-7T-CycleGAN

        parser.set_defaults(no_dropout=True)
        if is_train:
            parser.add_argument("--lambda_A", type=float, default=10, help="weight for cycle loss (A -> B -> A)") # Reduced to default 1 to match the paper
            parser.add_argument("--lambda_B", type=float, default=10, help="weight for cycle loss (B -> A -> B)") # Reduced to default 1 to match the paper
            parser.add_argument(
                "--lambda_identity",
                type=float,
                default=0.1,
            ...
            parser.add_argument("--lambda_per_s", type=float, default=0.1, help="weight for perceptual style loss") # This has been modified
    ...
    def forward(self):
        ...
        self.fake_B, self.content_B = self.netG_A(self.real_A)  # G_A(A)
        ...
        self.fake_A, self.content_A = self.netG_B(self.real_B)  # G_B(B)
    ...
    def backward_G(self):
        ...
        # Define lambdas for each loss compoenent
        ...
        # Compbine loss components into overall objective value
        self.loss_G = (
